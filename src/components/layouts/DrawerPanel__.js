...
import {deviceWide, fetchStore, getFileDownloadURL, getWidth, waitForData} from '../../utils';
...
class Header extends PureComponent {
    constructor(props) {
        ...
        this.state = {
            ...
            imgFailed: false,
            width:500,
            profileImage:images.default_ProPic
        }
    }
    async setProfile(){
        if(userStore.UserImage){
            await getImageBase64Query(userStore.UserImage)
                .then(result => {
                    this.setState({loadingImage: false, profileImage: result});
                })
                .catch(e =>{
...
export default class MenuBar extends PureComponent{
    ...
    handleDrawerClose = () => {
        this.setState({open:fa})
        this.props.onClose && this.props.onClose()
    };

    initDrawerData = async () => {
        await fetchStore();
        let nonGroupedItem = [];
        let items=[];
    ...
    componentDidMount() {
       /* if (this.state.menuItems.length==0){
            waitForData(this.initDrawerData);
        }*/
        this.setState({width:getWidth(),height:deviceWide(),isWide:false});
        this.props.onRef && this.props.onRef(this.initDrawerData);
    ...
    render(){
        ...
        if(userStore.Form.length!=this.FormLength){
            this.initDrawerData();
            this.FormLength=userStore.Form.length
        }
        ...
        return(
            <div >
                ...
                >
                    ...
                    />
                    {!this.state.isWide &&(
                        <div style={{padding:10}}  onClick={this.handleDrawerClose}>
                            {global.isRtl === 'rtl' ? <ChevronLeftIcon /> : <ChevronRightIcon />}
                        </div>
                    )}
                        ...
                        <SectionList
                            ...
                            renderHeader={(item, index) => (
                                <div style={{display:"flex"}}>
                                    <span key={index} style={{fontSize:12,color:gray}} >{item.title}</span>{/*ITS TITLE FOR SIDE BAR*/}
                                    <img
                    ...
                    <Divider />
                    <div onClick={this.logout}  style={{cursor: 'pointer', display:'flex',alignItems:'center',justifyContent:'center', width:'100%',height:50,padding:10,marginBottom:'5%' }}>
                        <img
