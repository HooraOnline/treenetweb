...
export default class ManualPay extends PureComponent {
                  ...
                  footer={
                      <TouchableOpacity
                          ...
                          style={{
                              ...
                              right:0,
                              padding:4,
                              backgroundColor: this.state.type === 1 ? primaryDark : this.checkValidation() ? primaryDark : borderSeparate,
                          }}
                      >
                          <Text style={{
                              fontFamily: Platform.OS === 'ios' ? 'IRANYekanFaNum-Bold' : 'IRANYekanBold(FaNum)',
                              fontSize: 16,
                              color: this.state.type === 1 ? 'white' : this.checkValidation() ? 'white' : subTextItem,
                              paddingVertical: 10,
                              textAlign: 'center',
                          }}>ذخیره</Text>
                ...
                <View style={{flex: 1, backgroundColor: bgScreen,paddingBottom:40}}>
                    ...
                    <View style={{flex: 1,position:'relative',}}>
                        <KeyboardAwareScrollView enableOnAndroid>
                            ...
                            >
                                <CardUnitInfo   style={{borderWidth:1,borderColor: borderLight}} bgColor={bgScreen} unitNumber={this.payInfo.user.UnitNumber} floorNumber={this.payInfo.user.FloorNumber} area={this.payInfo.user.Area}>
    ...
    animateSwipe = (value) => {
        Animated.timing(this.state.animatedSwipeClose, {
            toValue: value,
            duration: 10,
        }).start();
    };
    ...
    onPhotoSelector(isCamera) {
        alert('استفاده از امکانات سخت افزاری گوشی به زودی پیاده سازی می شود')
        return
    ...
    async adminPayment() {
        Keyboard.dismiss();
        if (this.state.type === CARD_TYPE && !this.checkValidation()) {
            if (!this.state.accListSelected) {
                this.setState({accListSelectedValidation: false});
                return;
            }
        ...
        if (this.state.type === CHECK_TYPE && !this.checkValidation()) {
            if (!this.state.checkDate) {
